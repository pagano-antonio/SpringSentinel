...
Run the Maven audit from the project root:
```bash
...
```groovy
plugins {
    id "io.github.pagano-antonio.spring-sentinel" version "2.0.2"
}
...
Run the Gradle audit from the project root:
```bash
gradle springSentinelAudit
```

For local development of this repository, the Gradle build can compile the shared core and the Gradle plugin together:
```bash
gradle build
```
...
Gradle: `build/spring-sentinel-reports/`
...
Build Tool: Maven 3.6.0+ or Gradle 8.x.
