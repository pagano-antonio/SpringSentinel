...
public class RuleConfigLoader {
    ...
    public RuleConfigLoader(Consumer<String> log) {
    ...
    public ResolvedConfig loadActiveRules(File customFile, String profileId) throws Exception {
        ...
        if (is == null) {
            throw new RuntimeException("default-rules.xml not found in classpath!");
        }

        Document defaultDoc = DocumentBuilderFactory.newInstance()
                .newDocumentBuilder()
                .parse(is);
        ...
        if (customFile != null && customFile.exists()) {
            log.accept("Loading custom rules from: " + customFile.getName());

            customDoc = DocumentBuilderFactory.newInstance()
                    .newDocumentBuilder()
                    .parse(customFile);
        }
    ...
    private void applyProfile(Document defaultDoc,
                              Document customDoc,
                              String profileId,
        ...
        if (profile == null) {
            log.accept("⚠️ Profile '" + profileId + "' not found!");
            return;
        }
        ...
        for (int i = 0; i < includes.getLength(); i++) {
            activeRules.add(
                    ((Element) includes.item(i)).getAttribute("rule")
            );
        }
        ...
        for (int i = 0; i < excludes.getLength(); i++) {
            activeRules.remove(
                    ((Element) excludes.item(i)).getAttribute("rule")
            );
        }
    ...
    private Element findProfileById(Document defaultDoc,
                                    Document customDoc,
                                    String id) {
        ...
        if (customDoc != null) {
            ...
            for (int i = 0; i < customProfiles.getLength(); i++) {
                ...
                if (p.getAttribute("id").equals(id)) {
                    return p;
                }
            }
        }
